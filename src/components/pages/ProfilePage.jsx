...
const fetchUserTagsById = async () => {
  const response = await axios.get(`https://nfz1b0p6-8787.euw.devtunnels.ms/${id}/tags`)
  
  setUserTags(response.data)
}
const fetchGallery = async () => {
      const response = await axios.get(`https://nfz1b0p6-8787.euw.devtunnels.ms/${id}/photos`)
...
const fetchProfileData = async () => {
    try {
        const response = await axios.get(`https://nfz1b0p6-8787.euw.devtunnels.ms/dates/availability/${id}`);
        ...
        const reviewsResponse = await axios.get('https://nfz1b0p6-8787.euw.devtunnels.ms/reviews');            
...
axios.put(`https://nfz1b0p6-8787.euw.devtunnels.ms/dates/availability/${id}`, {
...
axios.post('https://nfz1b0p6-8787.euw.devtunnels.ms/reviews', {
    ...
    <div className="row align-items-start">
      <div className="col-md  d-flex" id="left-column">
        ...
        <div className="container-left-column">
          ...
          <div className="left-side-container d-flex flex-column justify-content-center align-itens-center">
            <div>
              {users.map((user) =>
              user.id == id ? <h1 style={{color: "white"}} key={user.id}>{user.username}</h1> : null
              )}
            </div>
            <br />
            <br />
            <div className="tagsdiv">
              ...
              <div className="d-flex justify-content-center">
                ...
                <button type="button" class="btn btn-primary">
                  {utag} <span class="badge badge-light">4</span>
                </button>
                ))}
              </div>
            </div>
          ...
          <div className="topdiv-middle">
            <div>
              ...
              <iframe src={modifiedSpotifyProfileLink} width="100%" height="360" frameborder="0"
                allowtransparency="true" allow="encrypted-media" data-gtm-yt-inspected-6="true"></iframe> ) : (
                <div className="images-gallery-item">
                  <h5 className="container-spotify-widget p-5" style={{ color: "white" }}>Sorry, I dont have a Spotify profile to display! <i className="bi bi-emoji-frown-fill"></i></h5>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="col-md d-flex" id="right-column">
        ...
        <div className="middiv-right p-2 d-flex flex-column justify-content-center">
          <h1>Reviews</h1>
          {inputReviewerId == id ? (
            <div className="container-reviews d-flex justify-content-center align-items-center">
                <>
                  <ul className="review-list d-flex justify-content-center align-items-center p-0">
                    {currentReview ? (
                      <li key={currentReview.id} className="review-item text-center">
                        {currentReview.comment}
                      </li>
                    ): <p>There are no comments to show for this profile, be the first.</p>}
                  </ul>
                </>
            </div> 
            ) : (
            <div>
              ...
              <>
                <div>
                  <textarea className="new-comment w-100" value={newReview} onChange={handleReviewChange} placeholder="Write your review here..."  rows={4}/> 
                </div>
                <div>
                  <button className="button-78" onClick={handleSubmitReview}>Submit Review</button>
                  <button className="button-78" onClick={() => setShowReviewForm(false)}>Cancel</button>
                </div>
              </>
                ) : (
                  <>
